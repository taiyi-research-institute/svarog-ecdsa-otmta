...
但恶意 Bob 可以对每行 $h$ 用不同的 $b^{h}$ 来构造 $u$:
...
Bob 知道 $t^0_{*,j}$. 所以 $K_j^0$ 的值对他来说只有两种情况, 即:
* 假设 $s_{h}=0$, 那么 $K^0_j=\mathtt{Hash}(t^0_{*,j})$.
* 假设 $s_{h}=1$, 那么 $K^0_j=\mathtt{Hash}(t^0_{*,j} \oplus e_{h})$.

Alice 用真实的 $K^0_j = \mathtt{Hash}(q_{*,j})$ 加密某个具体内容.
Bob 收到密文后用两个候选密钥各解密一次, 能解出有意义内容的那个, 就揭示了 $s_{h}$ 的真实值.
每列换一个 $h$, 对不同的列 $j$ 重复此过程, $\kappa$ 次之后 Bob 掌握了完整的 $s$.

问题: 即使 Bob 不作恶, 他也能知道自己的 $b^*_j$. 这能够帮助他反推 Alice 的 $s_i$ ?

回答: 不能. 如果 Bob 采用诚实的 $b^*_j$, 那么 $K_j^0$ 的值对他来说就远远不止 2 种情况,
而是 $2^\kappa$ 种.

### 攻击二: 同时获取两条 OT 消息

通过攻击一得到 $s$ 之后, 对任意 OT 实例 $j$, Bob 均可算出两把密钥:
...
Bob 想进行 $m$ 次 OT, 持有选择向量 $b \in \mathbb{B}^m$. 他计算
...
问题全文: 如果 Bob 恰好蒙到了某一个 $s_{h}=0$ 的位置, 有什么后果?

首先, Bob 蒙对所有 $s$ 的概率极低. Alice 还有其他 $s_i=1$ 的位置防着 Bob.

其次, 如果 Bob 恰好在某个 $s_{h}=0$ 的那一行注入了 $b'$, 那么等式 "verify" 恒成立. 然而对于这种情况, Alice 算出的 $q_{i,*}$ 都恒等于 $t^0_{i,*}$. Bob 的篡改不会从 Alice 骗出 $s$ 的值, 也就是说 Bob 没有收益.


### 追问2: Bob 制造多次 abort, 还原出种子 $s$.

问题全文: Bob 可以有计划地在不同的 $h$ 注入不一致的 $b'$, 不论协议是否异常终止, 都能推断 $s_{h}$ 的值. 在 $\kappa$ 场别有用心的会话以后, Bob 就得到了所有的 $s_{h}$, 也就得到了整个 $s$ 向量.
...
* 要么学到 $s_{h}$. 但协议 abort, Bob 拿不到任何 OT 输出.
